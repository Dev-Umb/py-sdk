...
class ServiceManager:
    ...
    def init(self) -> bool:
        ...
        logger.info(ctx,"服务管理器初始化成功")
        return True
    
    def register_signal_handlers(self):
        """注册信号处理函数"""
        def signal_handler(sig, frame):
            logger.info(ctx,f"接收到信号 {sig}，正在退出...")
            self.cleanup()
    ...
    def register_service(self, service_name: str, port: int, 
        ...
        if success:
            ...
            logger.info(ctx,f"服务 {service_name}:{port} 注册成功")
        else:
            logger.error(ctx,f"服务 {service_name}:{port} 注册失败")
    ...
    def unregister_service(self, service_name: str, port: int,
        ...
        if service_key in self.registered_services:
            ...
            if result:
                ...
                logger.info(ctx,f"服务 {service_name}:{port} 注销成功")
                return True
            else:
                logger.error(ctx,f"服务 {service_name}:{port} 注销失败")
                return False
        else:
            # 如果未指定IP，获取本机IP
            if ip is None:
    ...
    def register_services_from_config(self) -> Tuple[int, int]:
        ...
        logger.info(ctx,f"从配置成功注册 {success_count}/{len(services)} 个服务")
        return success_count, len(services)
    
    def cleanup(self):
        """清理所有已注册的服务"""
        logger.info(ctx,"开始清理已注册的服务")
